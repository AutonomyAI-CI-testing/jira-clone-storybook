...
export const Title = ({
  ...
  // Determine if title/subtitle has reached max length to display warning color
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;
  // Show error state only if error prop is provided AND field is empty/whitespace
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));
  const requireSubtitleError =
    subtitleError && (subtitle.length === 0 || textAreOnlySpaces(subtitle));

  // Track focus to show/hide character count display
  const onFocus = () => {
  ...
  // Enforce character limit by preventing updates that exceed maxLength
  ...
  return (
    <div className="space-y-4">
      <div className="relative">
        ...
        {/* Character count shown only when focused, warns in red at max length */}
        {isFocus && (
          <span
            className={cx(
