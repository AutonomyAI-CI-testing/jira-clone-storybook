...
export const Title = ({
  ...
  // Title starts focused to encourage immediate input
  ...
  // Show error only when title is truly empty (not just whitespace)
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));

  // Factory for creating focus/blur handlers that respect read-only state
  const createFocusHandlers = (setFocusState: (focused: boolean) => void) => ({
    onFocus: () => {
  ...
  const titleHandlers = createFocusHandlers(setIsFocus);
  const subtitleHandlers = createFocusHandlers(setIsSubtitleFocus);

  // Prevent exceeding max length by rejecting updates that would overflow
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <TextareaAutosize
          ...
          readOnly={readOnly}
          onFocus={subtitleHandlers.onFocus}
          onBlur={subtitleHandlers.onBlur}
