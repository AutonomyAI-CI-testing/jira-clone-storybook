...
export const Title = ({
  ...
  // Show max length indicator when user is at or exceeds the character limit
  const isMaxLength = title.length >= maxLength;
  // Only display error message if an error is defined AND the field is empty or whitespace-only
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));

  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;
  // Only display subtitle error message if an error is defined AND the field is empty or whitespace-only
  ...
  // Prevent updating title if it would exceed max length (enforces character limit)
  const updateTitle = (newTitle: string) => {
    if (newTitle.length > maxLength) return;
    setTitle(newTitle);
  };

  // Prevent updating subtitle if it would exceed max length (enforces character limit)
