...
export const Title = ({
  ...
  // Track if fields are at max length to show warning indicator
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;
  // Only show error if validation fails (empty or whitespace-only content)
  const requireError =
  ...
  // Focus handlers manage visibility of character count display
  // In read-only mode, suppress focus state to prevent interaction feedback
  const onTitleFocus = () => {
  ...
  // Update handlers enforce max length by ignoring edits that exceed the limit
  // This prevents the text field from ever exceeding the defined max
  ...
  return (
    <div className="space-y-2">
      <div className="relative">
        ...
        {/* Character counter displayed only on focus, turns red when at max length */}
        {isTitleFocus && (
          <span
            className={cx(
