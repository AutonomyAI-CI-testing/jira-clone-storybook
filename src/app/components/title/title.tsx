...
export const Title = ({
  ...
  // Determine if text limits are reached for visual feedback
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;

  // Check if error states should be displayed (title/subtitle is empty or whitespace-only)
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));
  const requireSubtitleError =
    subtitleError && (subtitle.length === 0 || textAreOnlySpaces(subtitle));

  // Focus handlers - only update if not in read-only mode
  const onFocus = () => {
  ...
  // Update title - prevent setting if it exceeds max length (enforced at input level)
  ...
  // Update subtitle - prevent setting if it exceeds max length (enforced at input level)
  ...
  return (
    <div className="space-y-3">
      <div className="relative">
        ...
        {/* Display character count only when focused; turn red when at max length */}
        {isFocus && (
          <span
            className={cx(
      ...
      <div className="relative">
        ...
        {/* Display subtitle character count only when focused; turn red when at max length */}
        {isSubtitleFocus && (
          <span
            className={cx(
              "absolute right-0 top-full font-primary-light text-sm",
