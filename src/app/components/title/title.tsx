...
export const Title = ({
  ...
  // Computed state for UI styling and validation
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;
  // Show error only when required error state is set AND title is empty or whitespace-only
  const requireError =
  ...
  // Enforce max length constraints at the setter level to prevent user input beyond limit
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <TextareaAutosize
          ...
          textareaClassName={cx(
            "font-primary-light text-base text-font-subtlest"
          )}
        />
        {isSubtitleFocus && (
          <span
