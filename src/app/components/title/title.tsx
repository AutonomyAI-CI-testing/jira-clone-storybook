...
export const Title = ({
  ...
  // Title starts focused, subtitle starts unfocused
  ...
  // Show error only when error message is provided AND title is empty or whitespace-only
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));

  // Only show character count when field is focused (and not in readOnly mode)
  const onFocus = () => {
  ...
  const updateTitle = (newTitle: string) => {
    // Prevent exceeding maxLength by rejecting the update
  ...
  return (
    <div className="space-y-3">
      <div className="relative">
        {/* Title field with error state styling */}
        <TextareaAutosize
          name="title"
          value={title}
        ...
        {/* Character count shown only when focused, red when at/over limit */}
        {isFocus && (
          <span
            className={cx(
