...
export const Title = ({
  ...
  // Check if character limits are reached to change indicator color
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;
  
  // Error is shown only when error message is provided AND title is empty or whitespace-only
  const requireError =
  ...
  const updateTitle = (newTitle: string) => {
    // Enforce max length by rejecting updates that exceed limit
  ...
  return (
    <div className="space-y-2">
      <div className="relative">
        ...
        {/* Character counter shown only when title is focused */}
        {isFocus && (
          <span
            className={cx(
      ...
      <div className="relative">
        ...
        />
        {/* Character counter shown only when subtitle is focused */}
        {isSubtitleFocus && (
          <span
            className={cx(
              "absolute right-0 top-full font-primary-light text-sm",
