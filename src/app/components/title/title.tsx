...
/**
 * Title component for editing and displaying title and subtitle text.
 * Supports optional character limits, error states, and read-only mode.
 * Character counts are displayed when focused to help guide text input.
 */
export const Title = ({
  initTitle = "",
  readOnly,
  ...
  // isFocus controls visibility of character count indicators - only shown when focused
  const [isFocus, setIsFocus] = useState<boolean>(true);

  const isMaxLength = title.length >= maxLength;
  // Error is displayed only when the field is empty or contains only whitespace
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));

  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;
  // Same error logic applies to subtitle - error is shown only for empty/whitespace
  ...
  // Enforce max length for subtitle
