...
export const Title = ({
  ...
  // Show error when the error prop is set and the title is empty or only whitespace
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));

  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;
  // Show error when the subtitle error prop is set and the subtitle is empty or only whitespace
  ...
  // Prevent updating if new text exceeds max length
  ...
  // Conditionally render subtitle section if: it has content, requires error, or has custom placeholder
  const hasSubtitleProps =
    initSubtitle !== "" ||
    subtitleError ||
    subtitlePlaceholder !== "Write the subtitle";

  return (
    <div className={cx(hasSubtitleProps && "space-y-3")}>
