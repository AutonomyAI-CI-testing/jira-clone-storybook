...
// Default character limits for title and subtitle inputs
...
export const Title = ({
  ...
  // Only show error if provided and the field is empty/whitespace-only
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));
  const requireSubtitleError =
    subtitleError && (subtitle.length === 0 || textAreOnlySpaces(subtitle));

  // Focus handlers: show character counter when focused, but prevent focus in read-only mode
  const onFocus = () => {
  ...
  // Update handlers enforce max length constraints
  ...
  return (
    <div className="space-y-4">
      <div className="relative">
        ...
        {/* Character counter displays when focused, changes color at max length */}
        {isFocus && (
          <span
            className={cx(
