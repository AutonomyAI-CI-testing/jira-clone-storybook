...
export const Title = ({
  ...
  // Prevents submission with invalid content like "   "
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));

  // Focus handlers control character counter visibility
  // In readOnly mode, counter is never shown since user can't edit
  const onFocus = () => {
  ...
  const updateTitle = (newTitle: string) => {
    // Hard limit — prevent any input beyond maxLength
  ...
  const updateSubtitle = (newSubtitle: string) => {
    // Hard limit — prevent any input beyond subtitleMaxLength
