...
export const Title = ({
  ...
  // Character count has reached or exceeded the maximum allowed
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= maxLength;

  // Error is shown only when title is empty or contains only whitespace
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));

  // Show character counter when the field receives focus (only in edit mode)
  const onFocus = () => {
  ...
  // Prevent updates that would exceed the character limit
