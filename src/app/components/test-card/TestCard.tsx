/**
 * Gear/settings icon component
 */
const GearIcon = ({ className }: { className?: string }) => (
  <svg
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
  >
    <path
      d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15Z"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M19.6224 10.3954L18.5247 7.7448L20 6L18 4L16.2647 5.48295L13.5578 4.36974L12.9353 2H10.981L10.3491 4.40113L7.70441 5.51596L6 4L4 6L5.45337 7.78885L4.3725 10.4463L2 11V13L4.40111 13.6555L5.51575 16.2997L4 18L6 20L7.79116 18.5403L10.397 19.6123L11 22H13L13.6045 19.6132L16.2551 18.5155C16.6969 18.8313 18 20 18 20L20 18L18.5159 16.2494L19.6139 13.598L21.9999 12.9772L22 11L19.6224 10.3954Z"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

/**
 * Chevron icon for collapsible/expandable sections
 */
const ChevronIcon = ({ className }: { className?: string }) => (
  <svg
    width="20"
    height="20"
    viewBox="0 0 20 20"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
  >
    <path
      d="M15 12.5L10 7.5L5 12.5"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

/**
 * Info/help icon for tooltips and explanations
 */
const InfoIcon = ({ className }: { className?: string }) => (
  <svg
    width="14"
    height="14"
    viewBox="0 0 14 14"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
  >
    <circle cx="7" cy="7" r="6" stroke="currentColor" strokeWidth="1.2" />
    <path
      d="M7 10V7M7 4.5V4"
      stroke="currentColor"
      strokeWidth="1.2"
      strokeLinecap="round"
    />
  </svg>
);

/**
 * Form field with label and optional info icon
 */
const FormField = ({
  label,
  children,
}: {
  label: string;
  children: React.ReactNode;
}) => (
  <div>
    <div className="mb-2 flex items-center gap-2">
      <label className="text-[11.5px] font-semibold leading-[13.92px] text-[#a4a4a3]">
        {label}
      </label>
      <InfoIcon className="text-[#a4a4a3]" />
    </div>
    {children}
  </div>
);

export const TestCard = (): JSX.Element => {
  return (
    <div className="w-[340px] rounded-md bg-[#1a1a17] p-5">
      {/* Header with agent name and settings gear */}
      <div className="flex items-center justify-between">
        <h2 className="text-[13.5px] font-bold leading-[16.34px] text-[#b5b5b5]">
          UI magician Agent
        </h2>
        <GearIcon className="text-[#b5b5b5]" />
      </div>

      {/* Collapsed task description */}
      <div className="mt-4 flex items-center gap-2">
        <ChevronIcon className="flex-shrink-0 text-[#8b9291]" />
        <span className="text-[11.5px] font-semibold leading-[13.92px] text-[#8b9291]">
          From entire frame to a singl...
        </span>
      </div>

      {/* Spacer to create visual separation between sections */}
      <div className="mt-24" />

      {/* Expandable "Add New Design" section */}
      <div className="flex items-center gap-2">
        <ChevronIcon className="flex-shrink-0 text-[#b2b2b1]" />
        <span className="text-[13.5px] font-semibold leading-[16.34px] text-[#b2b2b1]">
          Add New Design
        </span>
      </div>

      {/* Personal Access Token input field */}
      <div className="mt-8">
        <FormField label="Personal Access Token">
          <div className="flex h-[36px] w-full items-center rounded-sm border-2 border-[#a5adad] bg-[#272822] px-3">
            <span className="text-[11.5px] font-semibold leading-[13.92px] text-[#737470]">
              figd_xxxxxxxxxxxxxxxxxx
            </span>
          </div>
        </FormField>
      </div>

      {/* Design URL input field */}
      <div className="mt-5">
        <FormField label="Design URL">
          <div className="flex h-[36px] w-full items-center rounded-sm border-[3px] border-[#929291] bg-[#272822] px-3">
            <span className="text-[10.5px] font-semibold leading-[12.71px] text-[#71726e]">
              https://www.figma.com/file/:
            </span>
          </div>
        </FormField>
      </div>

      {/* Action buttons */}
      <div className="mt-6 flex gap-5">
        <button className="flex h-[45px] w-[110px] items-center justify-center rounded bg-[#843a17] text-[12.5px] font-semibold leading-[13.92px] text-[#8c8078]">
          Awesome
        </button>
        <button className="flex h-[45px] w-[110px] items-center justify-center rounded bg-[#843a17] text-[12.5px] font-semibold leading-[13.92px] text-[#8c8078]">
          Prepare
        </button>
      </div>

      {/* Recent Breakdowns list section */}
      <div className="mt-16">
        <h3 className="text-[13.5px] font-semibold leading-[16.34px] text-[#b0b0b0]">
          Recent Breakdowns
        </h3>
      </div>

      {/* Footer with copyright */}
      <div className="border-gray-300 mt-6 border-t pt-3 text-center">
        <p className="text-gray-400 text-[11.5px]">© AutonomyAI</p>
      </div>
    </div>
  );
};
