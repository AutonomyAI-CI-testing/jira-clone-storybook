...
export const Title = ({
  ...
  // Display max-length indicator color (red) when at or near limit
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;

  // Only show error if an error message is provided AND the field is empty or contains only whitespace
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));
  const requireSubtitleError =
    subtitleError && (subtitle.length === 0 || textAreOnlySpaces(subtitle));

  // Track focus state to show/hide character counters during editing
  const onFocus = () => {
  ...
  // Prevent updating state if the new value exceeds max length
