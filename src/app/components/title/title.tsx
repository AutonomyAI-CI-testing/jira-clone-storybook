...
export const Title = ({
  ...
  // Show error when error message is provided and field is empty or only spaces
  const isTitleEmpty = title.length === 0 || textAreOnlySpaces(title);
  const showTitleError = error && isTitleEmpty;
  const isTitleMaxLength = title.length >= maxLength;

  const isSubtitleEmpty = subtitle.length === 0 || textAreOnlySpaces(subtitle);
  const showSubtitleError = subtitleError && isSubtitleEmpty;
