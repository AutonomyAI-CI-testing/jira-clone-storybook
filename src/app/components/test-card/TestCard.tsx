export const TestCard = () => {
  return (
    <div
      id="testElem"
      className="w-[254px] rounded bg-[#171717] px-5 py-5 font-sans"
    >
      {/* Header */}
      <div className="flex items-center justify-between">
        <span className="text-[13.5px] font-semibold leading-[16px] text-[#b5b5b5]">
          UI magician Agent
        </span>
        <img
          src="/images/test-card/settings-icon.svg"
          alt=""
          className="h-[14px] w-[16px]"
        />
      </div>

      {/* Collapsed row */}
      <div className="mt-8 flex items-center gap-1.5">
        <img
          src="/images/test-card/chevron-small-icon.svg"
          alt=""
          className="h-[5px] w-[8px]"
        />
        <span className="truncate text-[11.5px] font-semibold text-[#8b9291]">
          From entire frame to a singl...
        </span>
      </div>

      {/* Add New Design section */}
      <div className="mt-16">
        <div className="flex items-center gap-1.5">
          <img
            src="/images/test-card/chevron-icon.svg"
            alt=""
            className="h-[8px] w-[12px]"
          />
          <span className="text-[13.5px] font-semibold text-[#b2b2b1]">
            Add New Design
          </span>
        </div>

        <div className="mt-6">
          <div className="flex items-center gap-1">
            <span className="text-[11.5px] font-semibold text-[#a4a4a3]">
              Personal Access Token
            </span>
            <span className="flex h-3 w-3 items-center justify-center rounded-full border border-[#8c8c8c] text-[8px] text-[#8c8c8c]">
              i
            </span>
          </div>
          <div className="mt-2 rounded border border-[#a5adad] bg-[#272822] px-3 py-2.5">
            <span className="text-[11.5px] font-semibold text-[#737470]">
              figd_xxxxxxxxxxxxxxxxxx
            </span>
          </div>
        </div>

        <div className="mt-6">
          <div className="flex items-center gap-1">
            <span className="text-[11.5px] font-semibold text-[#a3a3a2]">
              Design URL
            </span>
            <span className="flex h-3 w-3 items-center justify-center rounded-full border border-[#8c8c8c] text-[8px] text-[#8c8c8c]">
              i
            </span>
          </div>
          <div className="mt-2 rounded border-2 border-[#929291] bg-[#272822] px-3 py-2.5">
            <span className="text-[10.5px] font-semibold text-[#71726e]">
              https://www.figma.com/file/:
            </span>
          </div>
        </div>

        <div className="mt-5 flex gap-2">
          <button
            type="button"
            className="flex flex-1 items-center justify-center gap-1.5 rounded bg-[#843a17] py-2.5 text-[11.5px] font-semibold text-[#8c8078]"
          >
            <img
              src="/images/test-card/awesome-icon.svg"
              alt=""
              className="h-[15px] w-[15px]"
            />
            Awesome
          </button>
          <button
            type="button"
            className="flex flex-1 items-center justify-center gap-1.5 rounded bg-[#843a17] py-2.5 text-[11.5px] font-semibold text-[#8c8078]"
          >
            <img
              src="/images/test-card/prepare-icon.svg"
              alt=""
              className="h-[15px] w-[15px]"
            />
            Prepare
          </button>
        </div>
      </div>

      {/* Recent Breakdowns */}
      <div className="mt-8">
        <span className="text-[13.5px] font-semibold text-[#b0b0b0]">
          Recent Breakdowns
        </span>
      </div>
    </div>
  );
};
