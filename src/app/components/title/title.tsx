...
export const Title = ({
  ...
  // Determine if max length warnings should be displayed
  const isMaxLength = title.length >= maxLength;
  const isSubtitleMaxLength = subtitle.length >= subtitleMaxLength;

  // Show error only if error prop is set AND the field is empty or contains only spaces
  const requireError =
    error && (title.length === 0 || textAreOnlySpaces(title));
  const requireSubtitleError =
    subtitleError && (subtitle.length === 0 || textAreOnlySpaces(subtitle));

  // Focus handlers control visibility of character count indicator
  const onFocus = () => {
  ...
  // Update handlers enforce max length constraint by preventing state updates that exceed limit
  ...
  return (
    <div className="space-y-4">
      {/* Title section with character count and error display */}
      ...
      {/* Subtitle section with character count and error display */}
      <div className="relative">
        <TextareaAutosize
          name="subtitle"
          value={subtitle}
