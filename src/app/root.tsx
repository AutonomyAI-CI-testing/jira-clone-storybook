...
export function ErrorBoundary() {
  ...
  if (isRouteErrorResponse(error)) {
    ...
    const message =
      error.status === 404
        ? "It seems that you have lost! Go to the main page"
    ...
    return (
      <html>
        <head>
          <title>
            {error.status === 404 ? "Ooops! Not found" : "Error occurred"}
          </title>
          <Meta />
          <Links />
        </head>
        <body>
          <div style={errorComponentStyle}>
            {error.status === 404 ? (
              <Error404 message={message} href="/" />
            ) : (
              <Error500 message={message} href="/" />
            )}
          </div>
        </body>
      </html>
